...
class DataSyncWorker @AssistedInject constructor(
    ...
    private suspend fun performPushSync(): Boolean {
        ...
        try {
            ...
            pushComplexChanges<Category>(
                ...
                updateApi = { category -> apiService.updateCategory(category.serverId!!.toLong(), category) },
            ...
            pushComplexChanges<Transaction>(
                ...
                updateApi = { transaction -> apiService.updateTransaction(transaction.serverId!!.toLong(), transaction) },
    ...
    private suspend fun performPullSync(): Boolean {
        ...
        try {
            pullEntityChanges<Book>(remoteApi = apiService.getUpdatedBooks(lastSyncTime), repository = bookRepository as SyncRepository<Book>)
            pullEntityChanges<Wallet>(remoteApi = apiService.getUpdatedWallets(lastSyncTime), repository = walletRepository as SyncRepository<Wallet>)
            pullEntityChanges<Category>(remoteApi = apiService.getUpdatedCategories(lastSyncTime), repository = categoryRepository as SyncRepository<Category>)
            pullEntityChanges<Transaction>(remoteApi = apiService.getUpdatedTransactions(lastSyncTime), repository = transactionRepository as SyncRepository<Transaction>)
